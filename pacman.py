...
class Ghost:
    ...
    def move_blinky(self):
        # r, l, u, d
        # vai virar sempre que colidir com paredes, caso contrário, continue em frente
        if self.direction == 0:
            if self.target[0] > self.x_pos and self.turns[0]:
                self.x_pos += self.speed
            elif not self.turns[0]:
    ...
    def move_inky(self):
        # r, l, u, d
        # vira para cima ou baixo em qualquer ponto para perseguir, mas esquerda e direita apenas em colisão
        if self.direction == 0:
            if self.target[0] > self.x_pos and self.turns[0]:
                self.x_pos += self.speed
            elif not self.turns[0]:
    ...
    def move_pinky(self):
        # r, l, u, d
        # vai virar à esquerda ou à direita sempre que for vantajoso, mas apenas para cima ou baixo na colisão
        if self.direction == 0:
            if self.target[0] > self.x_pos and self.turns[0]:
                self.x_pos += self.speed
            elif not self.turns[0]:
...
def draw_misc():
    ...
    if game_over:
        ...
        gameover_text = font.render(
            'Você Perdeu! Aperte Espaço para reiniciar!', True, 'red')
        screen.blit(gameover_text, (100, 300))
    if game_won:
        pygame.draw.rect(screen, 'white', [50, 200, 800, 300], 0, 10)
        pygame.draw.rect(screen, 'dark gray', [70, 220, 760, 260], 0, 10)
        gameover_text = font.render(
            'Você Venceu! Aperte Espaço para reiniciar!', True, 'green')
        screen.blit(gameover_text, (100, 300))
    ...
    for i in range(len(level)):
        for j in range(len(level[i])):
            ...
            if level[i][j] == 2 and not flicker:
                pygame.draw.circle(
                    screen, 'blue', (j * num2 + (0.5 * num2), i * num1 + (0.5 * num1)), 10)
            if level[i][j] == 3:
...
def check_position(centerx, centery):
    ...
    num2 = (WIDTH // 30)
    num3 = 15
    # verifique as colisões com base no centro x e y do jogador +/- número das paredes
    if centerx // 30 < 29:
        if direction == 0:
...
while run:
    ...
    # adicionado se os fantamas comidos com poder foram comidos
    if not powerup:
    ...
    if powerup and player_circle.colliderect(blinky.rect) and not blinky.dead and not eaten_ghost[0]:
        blinky_dead = True
        eaten_ghost[0] = True
        score += (2 ** eaten_ghost.count(True)) * 20
...
# efeitos sonoros, reinício e mensagens vencedoras, depois?
